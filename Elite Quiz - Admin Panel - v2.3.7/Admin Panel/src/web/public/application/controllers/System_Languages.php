...
class System_Languages extends CI_Controller
{
    ...
    public function index()
    {
        if (!$this->session->userdata('isLoggedIn')) {
            ...
            // Check if this is being loaded in an iframe (for unified languages page)
            $is_iframe = $this->input->get('iframe') == '1' || $this->input->get('embed') == '1';
            
            if ($is_iframe) {
                // Load only the content without header/footer
                $this->load->view('system_languages_content');
            } else {
    ...
    {
        // Method to return just the content for embedding
        if (!$this->session->userdata('isLoggedIn')) {
            redirect('/');
        } else {
            if (!$this->session->userdata('authStatus')) {
                ...
                // Handle POST actions same as index
                if ($this->input->post('btnadd')) {
                    if (!has_permissions('create', 'system_languages')) {
                        $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                    } else {
                    ...
                    redirect('system-languages/content?iframe=1');
                } else if ($this->input->post('btnupdate')) {
                    if (!has_permissions('update', 'system_languages')) {
                        $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                    } else {
