...
class AuthRepository {
  ...
  Future<({bool isNewUser, User user})> signInUser(
    ...
    try {
      log('🔵 [Auth Repo] Starting signInUser with provider: ${authProvider.name}');
      ...
      log('🔵 [Auth Repo] User UID: $userUid, Email: $userEmail, Name: $userName');

      /// checks in panel
      var userExists = !isNewUser;

      if (authProvider == AuthProviders.email) {
        log('🔵 [Auth Repo] Checking if user exists (email provider)...');
        userExists = await _authRemoteDataSource.isUserExist(userUid);
        log('🔵 [Auth Repo] User exists: $userExists');
      }

      if (!userExists) {
        log('🔵 [Auth Repo] User does not exist, registering new user...');
        isNewUser = true;
        ...
        log('✅ [Auth Repo] User registered successfully');

        await AuthLocalDataSource.setJwtToken(
          registeredUser['api_token'].toString(),
        );
        log('✅ [Auth Repo] JWT token saved');
      } else {
        log('🔵 [Auth Repo] User exists, getting JWT token...');
        final jwtToken = await _authRemoteDataSource.getJWTTokenOfUser(
          firebaseId: userUid,
          type: authProvider.name,
        );
        log('✅ [Auth Repo] JWT token retrieved');

        await AuthLocalDataSource.setJwtToken(jwtToken);
        log('🔵 [Auth Repo] Updating FCM ID...');
        await _authRemoteDataSource.updateFcmId(
          firebaseId: userUid,
          userLoggingOut: false,
        );
        log('✅ [Auth Repo] FCM ID updated');
      }

      log('✅ [Auth Repo] signInUser completed successfully');
      return (user: user, isNewUser: isNewUser);
    } catch (e, stackTrace) {
      log('❌ [Auth Repo] Error in signInUser: $e', error: e, stackTrace: stackTrace);
      await signOut(authProvider);
      // If it's already an ApiException, rethrow it; otherwise convert to default error
      if (e is ApiException) {
        rethrow;
      } else {
        log('❌ [Auth Repo] Non-ApiException caught: ${e.runtimeType} - $e');
